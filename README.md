...
* **물리 엔진 & 사운드 피드백:** 단순한 슬라이딩을 넘어, 조각이 고무줄처럼 쫀득하게 빨려 들어가는 후크의 법칙(Spring Physics) 물리 애니메이션과 Web Audio API 기반의 효과음 구현.
...
## 💡 플레이 방법
1. 카메라를 향해 양손으로 네모난 프레임(Frame)을 만듭니다.
2. 양손의 검지와 엄지를 꼬집어(Pinch) 찰칵! 캡처합니다.
3. 꼬집기(Pinch)로 퍼즐을 집어 들고, 빈 공간에 드래그 앤 드롭으로 퍼즐을 맞춥니다.
4. 제한 시간 내에 최대한 높은 레벨에 도달해 리더보드에 이름을 남기세요!
5. 화면 리셋이 필요하다면 주먹(Fist)을 1.5초간 꽉 쥐고 있으면 됩니다.